...
function PublicBlogPost({ title, author, date, content }) {
  return (
    <div className="blog-post">
      <h2>{title}</h2>
      <i className="meta">By {author} on {date}</i>
      <p>{content}</p>
    </div>
  );
}
  ...
  return (
    <div className="blog-list">
      {blogPosts.map((post, index) => {
        // Render PrivateBlogPost for private posts and PublicBlogPost for public posts
        if (post.isPrivate) {
          return (
            <PrivateBlogPost
              key={index}
              ...
              author={post.author}
              date={post.date}
              content="The content of this post is private" // Add message for private content
            />
          );
        } else {
          return (
...
function Footer({year}) {
 return(
  <footer>
  <p> {year} &copy; My Blog. All rights reserved.</p>
  </footer>
 )
};
